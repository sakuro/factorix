...
- On WSL, the Factorio installation directory resolved from a non-default Steam library folder stayed in Windows path form (e.g. `D:\SteamLibrary\...`) instead of being converted to `/mnt/d/...`, so `GameExecutablePath`/`GameDataDir` could return an unusable path (#199)
