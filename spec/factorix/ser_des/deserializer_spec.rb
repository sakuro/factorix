...
RSpec.describe Factorix::SerDes::Deserializer do
  ...
  describe "#read_game_version" do
    let(:binary_data) { "\x01\x00\x02\x00\x03\x00\x04\x00" }

    it "returns a GameVersion" do
      expect(deserializer.read_game_version).to eq(Factorix::SerDes::GameVersion.new(1, 2, 3, 4))
    end
  end

  describe "#read_mod_version" do
    let(:binary_data) { "\x01\x02\x03" }

    it "returns a MODVersion" do
      expect(deserializer.read_mod_version).to eq(Factorix::SerDes::MODVersion.new(1, 2, 3))
    end
  end
  ...
  describe "#read_optim_u16" do
    ...
    context "when leading byte is 0xFF" do
      ...
      it "returns 2 bytes after the leading byte" do
  ...
  describe "#read_str" do
    context "when leading byte is 0xFF" do
      ...
      it "reads string of bytesize designated in u32 after the leading byte" do
        expect(deserializer.read_str).to eq("x" * 300)
      end

      it "reads long UTF-8 multibyte string correctly" do
    ...
    context "when leading byte is not 0xFF" do
      ...
      it "reads string of bytesize designated by the leading byte itself" do
        expect(deserializer.read_str).to eq("hello, world")
      end

      it "reads UTF-8 multibyte string with correct bytesize" do
  ...
  describe "#read_long" do
    let(:binary_data) { "\xff\xff\xff\xff\xff\xff\xff\xff" }

    it "returns signed long integer" do
      expect(deserializer.read_long).to eq(-1)
    end

    context "with not enough bytes left" do
      let(:binary_data) { "\xff\xff\xff\xff\xff\xff\xff" }

      it "raises EOFError" do
        expect { deserializer.read_long }.to raise_error(EOFError)
      end
    end
  end

  describe "#read_unsigned_long" do
    let(:binary_data) { "\xff\xff\xff\xff\xff\xff\xff\xff" }

    it "returns unsigned long integer" do
      expect(deserializer.read_unsigned_long).to eq(0xFFFFFFFFFFFFFFFF)
    end
    ...
    context "with not enough bytes left" do
      ...
      it "raises EOFError" do
        expect { deserializer.read_unsigned_long }.to raise_error(EOFError)
      end
    end
  end
