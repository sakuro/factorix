# frozen_string_literal: true

RSpec.describe Factorix::Runtime::Windows do
  subject(:runtime) { Factorix::Runtime::Windows.new(path:) }

  let(:path) {
    instance_double(
      Factorix::Runtime::Windows::WindowsPath,
      app_data: Pathname("C:/Users/wube/AppData/Roaming"),
      local_app_data: Pathname("C:/Users/wube/AppData/Local")
    )
  }

  describe "#user_dir" do
    it "returns APPDATA/Factorio" do
      expect(runtime.user_dir).to eq(Pathname("C:/Users/wube/AppData/Roaming/Factorio"))
    end
  end

  describe "#mods_dir" do
    it "returns user_dir/mods" do
      expect(runtime.mods_dir).to eq(Pathname("C:/Users/wube/AppData/Roaming/Factorio/mods"))
    end
  end

  describe "#player_data_path" do
    it "returns user_dir/player-data.json" do
      expect(runtime.player_data_path).to eq(Pathname("C:/Users/wube/AppData/Roaming/Factorio/player-data.json"))
    end
  end

  describe "#xdg_cache_home_dir" do
    context "when XDG_CACHE_HOME is not set" do
      before do
        allow(ENV).to receive(:key?).with("XDG_CACHE_HOME").and_return(false)
      end

      it "returns LOCALAPPDATA" do
        expect(runtime.xdg_cache_home_dir).to eq(Pathname("C:/Users/wube/AppData/Local"))
      end
    end

    context "when XDG_CACHE_HOME is set" do
      before do
        allow(ENV).to receive(:key?).with("XDG_CACHE_HOME").and_return(true)
        allow(ENV).to receive(:fetch).with("XDG_CACHE_HOME").and_return("C:/custom/cache")
      end

      it "returns the custom path" do
        expect(runtime.xdg_cache_home_dir).to eq(Pathname("C:/custom/cache"))
      end
    end
  end

  describe "#xdg_config_home_dir" do
    context "when XDG_CONFIG_HOME is not set" do
      before do
        allow(ENV).to receive(:key?).with("XDG_CONFIG_HOME").and_return(false)
      end

      it "returns APPDATA" do
        expect(runtime.xdg_config_home_dir).to eq(Pathname("C:/Users/wube/AppData/Roaming"))
      end
    end
  end

  describe "#xdg_data_home_dir" do
    context "when XDG_DATA_HOME is not set" do
      before do
        allow(ENV).to receive(:key?).with("XDG_DATA_HOME").and_return(false)
      end

      it "returns LOCALAPPDATA" do
        expect(runtime.xdg_data_home_dir).to eq(Pathname("C:/Users/wube/AppData/Local"))
      end
    end
  end
end
