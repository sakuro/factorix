...
RSpec.describe Factorix::Runtime::UserConfigurable do
  # Create a test runtime class with UserConfigurable prepended
  let(:test_runtime_class) do
    ...
    end.tap {|klass| klass.prepend(Factorix::Runtime::UserConfigurable) }
  end
  ...
  describe "#executable_path" do
    ...
    context "when not configured and auto-detection fails" do
      let(:failing_runtime_class) do
        Class.new(Factorix::Runtime::Base).tap {|klass|
          klass.prepend(Factorix::Runtime::UserConfigurable)
        }
      end
