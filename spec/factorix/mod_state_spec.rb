...
RSpec.describe Factorix::MODState do
  describe "#initialize" do
    ...
    it "creates a MODState with enabled and version" do
      ...
      state = Factorix::MODState.new(enabled: false, version:)
  ...
  describe "#enabled?" do
    ...
    it "returns false when disabled" do
      state = Factorix::MODState.new(enabled: false)
      expect(state.enabled?).to be(false)
    end
  end
  ...
  describe "#version" do
    it "returns the version" do
      ...
      state = Factorix::MODState.new(enabled: true, version:)
      expect(state.version).to eq(version)
    end
  ...
  describe "equality" do
    ...
    it "considers two MODStates with same values as equal" do
      state1 = Factorix::MODState.new(enabled: true, version:)
      state2 = Factorix::MODState.new(enabled: true, version:)
      expect(state1).to eq(state2)
    end

    it "considers two MODStates with different enabled values as not equal" do
      state1 = Factorix::MODState.new(enabled: true, version:)
      state2 = Factorix::MODState.new(enabled: false, version:)
    ...
    it "considers two MODStates with different versions as not equal" do
      ...
      state1 = Factorix::MODState.new(enabled: true, version:)
      state2 = Factorix::MODState.new(enabled: true, version: other_version)
      expect(state1).not_to eq(state2)
    end
  end
